...
"""
Day 11 — Circuit Breaker Panel Complete + Dashboard Polish (Tushar).

Tushar's guide's end-of-day BLOCKER is a live, physical test: block the
...
def test_real_nemotron_rate_limit_falls_back_to_groq_and_opens_breaker(triage):
    ...
    Confirms both halves of what Tushar's panel + Workflow DAG need:
    ...
    is Day 12 scope per Tushar's own guide, not built yet)."""
    from fastapi.testclient import TestClient
    from api.main import app

    client = TestClient(app)
    loop_event = {"worker_id": "worker-tushar-11", "similarity": 0.95, "consecutive_count": 3, "error_hash": "rate_limited"}
